...
1. **快照粒度**：按「工具」整体（每次分发前快照整个工具技能目录）还是按「技能」增量？草案取**按工具整体**——回滚时间线简单直观，代价是文件重复；MVP 技能量小可接受
2. **回滚后是否留新快照**：草案不留（回滚本身再留一份会无限膨胀），回滚前工具端内容进 `.trash` 兜底
3. **一键全发的范围**：分发该工具 `targets` 标记的全部技能，还是 UI 当前勾选全集？（草案：targets 全集，勾选是逐项路径）
4. **说明文件分发**：复用 deploy 流程（内容实体 + 工具映射路径）但独立渲染（无 frontmatter 注入），草案如此
5. **保留 N 默认值**：草案默认 10，可配
6. **WorkBuddy 未配置路径时**：状态列显示「未接入」且不可分发（草案），还是允许用户占位填写？
